...
import { getVietnamDate, isRegistrationLocked } from '@/lib/time';
...
export async function GET(request) {
  ...
  try {
    ...
    const rows = await sql`
      SELECT id, so_danh_bo, nhom_phu_trach, sl_cnv, sl_nha_thau, thoi_gian_nhap
      FROM dang_ky_suat_an
      WHERE so_danh_bo = ${id} AND ngay_dang_ky = ${date}::date
...
export async function POST(request) {
  ...
  const { soDanhBo, group, cnv, contractor } = checked.value;
    ...
    const result = await sql`
      WITH previous AS MATERIALIZED (
        ...
        INSERT INTO dang_ky_suat_an (
          so_danh_bo, ngay_dang_ky, sl_cnv, sl_nha_thau, nhom_phu_trach, thoi_gian_nhap
        )
        VALUES (${soDanhBo}, ${date}::date, ${cnv}, ${contractor}, ${group}, CURRENT_TIMESTAMP)
        ON CONFLICT (so_danh_bo, ngay_dang_ky, nhom_phu_trach)
        DO UPDATE SET
          sl_cnv = EXCLUDED.sl_cnv,
          sl_nha_thau = EXCLUDED.sl_nha_thau,
          thoi_gian_nhap = CURRENT_TIMESTAMP
        RETURNING id, so_danh_bo, nhom_phu_trach, sl_cnv, sl_nha_thau, thoi_gian_nhap
      ), audited AS (
        INSERT INTO lich_su_dang_ky_suat_an (
          dang_ky_id, so_danh_bo, nhom_phu_trach,
          sl_cnv_truoc, sl_cnv_sau, sl_nha_thau_truoc, sl_nha_thau_sau,
          hanh_dong, dia_chi_ip
        )
        SELECT
          u.id, u.so_danh_bo, u.nhom_phu_trach,
