...
export function validateRegistration(body) {
  ...
  const ngayDangKy = String(body?.ngay_dang_ky ?? '').trim();
  const cnv = parseQuantity(body?.sl_cnv);
  const contractor = parseQuantity(body?.sl_nha_thau);
  const ghiChu = String(body?.ghi_chu ?? '').trim().slice(0, NOTE_MAX_LENGTH) || null;
  ...
  if (!isIsoDate(ngayDangKy)) return { error: 'Ngày ăn không hợp lệ.' };
  ...
  return { value: { soDanhBo, group, loaiSuat, ngayDangKy, cnv, contractor, ghiChu } };
}

export function validateAdminRegistrationEdit(body) {
  const ngayDangKy = String(body?.ngay_dang_ky ?? '').trim();
  if (!isIsoDate(ngayDangKy)) return { error: 'Ngày ăn không hợp lệ.' };
  const cnv = parseQuantity(body?.sl_cnv);
  const contractor = parseQuantity(body?.sl_nha_thau);
  ...
  if (cnv === 0 && contractor === 0) return { error: 'Ít nhất một loại suất ăn phải lớn hơn 0.' };
  const ghiChu = String(body?.ghi_chu ?? '').trim().slice(0, NOTE_MAX_LENGTH) || null;
  return { value: { ngayDangKy, cnv, contractor, ghiChu } };
}
